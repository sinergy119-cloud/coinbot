...
│   ├── session.ts        # 세션 관리
│   ├── admin.ts          # isAdmin() 판정
│   ├── telegram.ts       # 텔레그램 메시지 발송 (IPv4 강제)
│   └── supabase.ts       # Supabase 서버 클라이언트
└── components/
    ├── ErrorBoundary.tsx       # React 에러 바운더리 (흰 화면 방지)
    ├── IntroOverlay.tsx        # 서비스 소개 오버레이 (최초 접속 시)
...
| 파일 | 역할 |
|------|------|
| `src/lib/crawlers/execute.ts` | 크롤링 공통 실행 로직 — cron·수동 모두 이 함수를 호출 |
| `src/lib/crawlers/retry.ts` | 지수 백오프 재시도 유틸리티 (`withRetry`) |
| `src/lib/crawlers/keywords.ts` | 수집 키워드 설정 (INCLUDE_KEYWORDS / EXCLUDE_KEYWORDS) |
| `src/lib/crawlers/bithumb.ts` | 빗썸 공식 API 크롤러 |
| `src/lib/crawlers/upbit.ts` | 업비트 비공식 API 크롤러 (❌ EC2 IP Cloudflare 차단) |
...
| `src/app/api/admin/run-crawl/route.ts` | 수동 즉시 수집 API (관리자 세션 인증) |
| `src/app/api/admin/crawled-events/route.ts` | 관리자 검토 API (승인/거절) |
| `src/app/api/admin/crawler-keywords/route.ts` | 키워드 관리 API |
| `src/app/api/admin/crawl-logs/route.ts` | 수집 이력 조회 API (최근 20건) |
| `src/app/api/admin/test-telegram/route.ts` | 텔레그램 발송 테스트 API |
...
```bash
# 포트 3000, CRON_SECRET 환경변수 필요
pm2 start "curl -s -X POST http://localhost:3000/api/cron/crawl-events \
...
- `withRetry(fn, { retries: 2, delayMs: 1000, label: 'BITHUMB' })`
- 실패 시: 1초 → 2초 → 4초 대기 후 재시도 (지수 백오프)
- 빗썸·코인원·코빗·고팍스 4개 크롤러에 적용 (업비트는 차단으로 의미 없음)

#### 5분 중복 실행 방지

- `executeCrawl()` 진입 시 `crawl_logs` 테이블에서 최근 5분 내 실행 여부 확인
- 중복이면 `{ skipped: true, reason: '5분 이내 재실행 불가' }` 반환

#### 키워드 유지보수

- 관리자 UI(`수집 이벤트` 탭)에서 포함/제외 키워드를 직접 추가·삭제 가능
- `crawler_keywords` 테이블에 저장 (`src/app/api/admin/crawler-keywords/route.ts`)
- 수집 실행 시 DB에서 실시간으로 읽어옴 (재배포 불필요)

#### 거래소별 수집 방식

- **빗썸/고팍스**: 공식 REST API (JSON 응답)
- **업비트**: 비공식 API (`api-manager.upbit.com`) — ❌ EC2 IP Cloudflare 차단, 해결 전까지 수집 불가
