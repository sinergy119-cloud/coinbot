...
export async function GET(req: NextRequest) {
  ...
  if (!stateCheck.ok) {
    const fromAppGuess = (searchParams.get('state') ?? '').startsWith('app_')
  ...
  if (existingUser) {
    ...
    const dest = (existingUser.is_admin && !fromApp) ? '/?welcome=google' : '/app?welcome=google'
  ...
  if (email) {
    ...
    if (emailUser) {
      ...
      const dest = (emailUser.is_admin && !fromApp) ? '/?welcome=google' : '/app?welcome=google'
      return Response.redirect(`${origin}${dest}`)
    }
  }
