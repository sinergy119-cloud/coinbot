        ...
        <div className="flex-1 overflow-y-auto p-5">
          ...
          {tab === 'new' && (
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                />
                <p className="mt-1 text-[10px] text-gray-400 text-right">{title.length}/100</p>
              </div>
              ...
              <div>
                ...
                />
                <p className="mt-1 text-[10px] text-gray-400 text-right">{content.length}/2000</p>
              </div>
          ...
          {tab === 'list' && (
            <div className="space-y-2">
              {inquiries.length === 0 && (
                <p className="text-center text-sm text-gray-400 py-8">등록한 문의가 없습니다.</p>
              )}
              {inquiries.map((inq) => {
                ...
                return (
                  <div key={inq.id} className={`rounded-lg border p-3 ${answered ? 'border-green-200 bg-green-50/30' : 'border-gray-200 bg-white'}`}>
                    <div className="flex items-center gap-2 mb-1.5 flex-wrap">
                      ...
                      <span className="ml-auto text-[10px] text-gray-400">{toKST(inq.created_at)}</span>
                    </div>
                    ...
                    {answered && inq.admin_reply && (
                      <div className="mt-2 rounded-lg bg-white border border-green-200 p-2.5">
                        ...
                        {inq.answered_at && (
                          <p className="mt-1 text-[10px] text-gray-400">{toKST(inq.answered_at)}</p>
                        )}
                      </div>
                    )}
                  </div>
